 ...
 * Description: Water Savings Calculator page with fully functional JS calculator. All labels and copy editable via ACF.
 */
...
$coeff_note     = wcs_field( 'calc_coefficients_note', 'The following irrigation demand coefficients are used to estimate efficient water requirements by plant type. These are planning-level values based on typical UAE climate conditions and must be validated against municipality, QCC, or project-specific data for final engineering use.' );
...
.input-wrap input:focus, .input-wrap select:focus { outline: none; border-color: var(--color-primary); box-shadow: 0 0 0 3px rgba(0,174,203,0.1); }
...
.results-header { padding: var(--space-6) var(--space-8); background: rgba(0,174,203,0.05); border-bottom: 1px solid var(--color-border); }
...
.result-row--highlight .result-label { color: var(--color-text); font-weight: 600; font-size: var(--text-sm); }
.result-row--highlight .result-value { color: var(--color-primary); font-size: var(--text-xl); }
.results-divider { height: 1px; background: var(--color-divider); margin: var(--space-2) 0; }
.results-savings-box { background: rgba(0,174,203,0.06); border: 1px solid var(--color-border-accent); border-radius: var(--radius-lg); padding: var(--space-6); margin: var(--space-2) 0; }
...
<div class="calc-page">
  ...
  <div class="calc-header">
    <div class="container container--default">
      ...
      <h1 style="font-size: var(--text-2xl); font-weight: 700; color: var(--color-text); letter-spacing: -0.02em; line-height: 1.1; margin-bottom: 0.75rem;">
        <?php echo esc_html( $page_heading ); ?>
      </h1>
      <p style="font-size: var(--text-base); color: var(--color-text-muted); max-width: 58ch;">
      ...
      <div class="calc-layout">
        ...
        <div class="calc-form-card">
            ...
            <div class="calc-fields">
              <div class="calc-field calc-field--full">
                ...
                <div class="input-wrap">
                  <select id="calc-property-type" name="property_type">
              ...
              <div class="calc-field calc-field--full">
                ...
                <div class="input-wrap">
                  <input type="number" id="calc-total-area" placeholder="e.g. 8000" min="0" step="100" />
          ...
          <div class="calc-form-section">
            ...
            <p style="font-size: var(--text-xs); color: var(--color-text-faint); margin-bottom: var(--space-5); max-width: none;">Enter quantities/areas for each plant type present on your site. Leave blank if not applicable. Areas should sum approximately to your total landscaped area.</p>
            <div class="calc-fields">
              <div class="calc-field">
                <label for="calc-palms">Palm Trees</label>
                <div class="input-wrap">
                  <input type="number" id="calc-palms" placeholder="0" min="0" step="1" />
              ...
              <div class="calc-field">
                ...
                <div class="input-wrap">
                  <input type="number" id="calc-trees" placeholder="0" min="0" step="1" />
                  <span class="unit">no.</span>
                </div>
              </div>
              <div class="calc-field">
                ...
                <div class="input-wrap">
                  <input type="number" id="calc-shrubs" placeholder="0" min="0" step="100" />
                  <span class="unit">m²</span>
                </div>
              </div>
              <div class="calc-field">
                ...
                <div class="input-wrap">
                  <input type="number" id="calc-groundcover" placeholder="0" min="0" step="100" />
                  <span class="unit">m²</span>
                </div>
              </div>
              <div class="calc-field calc-field--full">
                ...
                <div class="input-wrap">
                  <input type="number" id="calc-turf" placeholder="0" min="0" step="100" />
                  <span class="unit">m²</span>
                </div>
              </div>
            </div>
          ...
          <div class="calc-form-section">
            ...
            <div class="calc-fields">
              <div class="calc-field calc-field--full">
                ...
                <div class="input-wrap">
                  <select id="calc-utility" name="utility">
              ...
              <div class="calc-field">
                <label for="calc-monthly-bill">Monthly Water Bill</label>
                <div class="input-wrap">
                  <input type="number" id="calc-monthly-bill" placeholder="e.g. 35000" min="0" step="500" />
              ...
              <div class="calc-field">
                ...
                <div class="input-wrap">
                  <input type="number" id="calc-irrigation-pct" placeholder="e.g. 60" min="0" max="100" step="5" />
                  <span class="unit">%</span>
                </div>
                <div class="hint">Estimated % of total bill attributable to outdoor irrigation</div>
              </div>
              <div class="calc-field calc-field--full">
                <label for="calc-water-source">Current Irrigation Source</label>
                <div class="input-wrap">
                  <select id="calc-water-source" name="water_source">
                    <option value="potable" selected>Potable (treated municipal water)</option>
                    <option value="recycled">Recycled / Treated Sewage Effluent</option>
                    <option value="mixed">Mixed (potable + recycled)</option>
            ...
            <div class="tariff-info">
              <strong style="color: var(--color-text-muted);">Tariff reference:</strong>
              <?php echo esc_html( $tariff_info ); ?>
            </div>
          </div>
          ...
          <div class="calc-form-section">
            ...
            <p style="font-size: var(--text-xs); color: var(--color-text-faint); margin-bottom: var(--space-3); max-width: none;">
              <?php echo esc_html( $coeff_note ); ?>
            </p>
              ...
              <table class="assumptions-table">
                <thead>
                  <tr>
                    <th>Plant Type</th>
                    <th>Unit</th>
                    <th style="text-align:right;">Efficient Demand</th>
                ...
                <tbody>
                  <tr>
                    <td>Palms</td>
                    <td>per tree/month</td>
                    <td style="text-align:right;"><input type="number" class="editable" id="coeff-palms" value="<?php echo esc_attr( $c_palms ); ?>" step="0.1" min="0" title="m³ per palm per month" /> m³</td>
                    <td style="text-align:right; color: var(--color-text-faint);">Planning est.</td>
                  </tr>
                  <tr>
                    <td>Trees</td>
                    <td>per tree/month</td>
                    <td style="text-align:right;"><input type="number" class="editable" id="coeff-trees" value="<?php echo esc_attr( $c_trees ); ?>" step="0.1" min="0" title="m³ per tree per month" /> m³</td>
                    <td style="text-align:right; color: var(--color-text-faint);">Planning est.</td>
                  </tr>
                  <tr>
                    <td>Shrubs</td>
                    <td>per m²/month</td>
                    <td style="text-align:right;"><input type="number" class="editable" id="coeff-shrubs" value="<?php echo esc_attr( $c_shrubs ); ?>" step="0.001" min="0" title="m³ per m² per month" /> m³</td>
                    <td style="text-align:right; color: var(--color-text-faint);">Planning est.</td>
                  </tr>
                  <tr>
                    <td>Ground Cover</td>
                    <td>per m²/month</td>
                    <td style="text-align:right;"><input type="number" class="editable" id="coeff-groundcover" value="<?php echo esc_attr( $c_groundcover ); ?>" step="0.001" min="0" title="m³ per m² per month" /> m³</td>
                    <td style="text-align:right; color: var(--color-text-faint);">Planning est.</td>
                  </tr>
                  <tr>
                    <td>Grass / Turf</td>
                    <td>per m²/month</td>
                    <td style="text-align:right;"><input type="number" class="editable" id="coeff-turf" value="<?php echo esc_attr( $c_turf ); ?>" step="0.001" min="0" title="m³ per m² per month" /> m³</td>
                    <td style="text-align:right; color: var(--color-text-faint);">Planning est.</td>
                  </tr>
                </tbody>
              </table>
              <p style="font-size: 10px; color: var(--color-text-faint); margin-top: var(--space-3); font-style: italic;">Values are editable for planning scenarios. Final coefficients must be confirmed against municipality approvals, QCC data, or project-specific engineering assessment. WCS does not warrant these values for procurement or contractual use.</p>
            </div>
          </div>

          <!-- Calculate button -->
          <div class="calc-form-section" style="border-bottom: none;">
        ...
        <div>
          <div class="results-card" id="resultsCard">
            <div class="results-header">
              <div class="results-title">Estimated Savings</div>
            </div>

            <!-- Empty state -->
            <div class="results-empty" id="resultsEmpty">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="var(--color-border)" stroke-width="1.5" aria-hidden="true">
            ...
            <div class="results-body" id="resultsBody" style="display: none;">
              <div class="result-row">
                <span class="result-label">Est. current irrigation cost</span>
                <span class="result-value" id="res-current-cost">—</span>
              </div>
              <div class="result-row">
                <span class="result-label">Est. efficient irrigation demand</span>
                <span class="result-value" id="res-demand-volume">—</span>
              </div>
              <div class="results-divider" aria-hidden="true"></div>
              <div class="result-row">
                <span class="result-label">Est. efficient irrigation cost</span>
                <span class="result-value" id="res-efficient-cost">—</span>
              </div>
              <div class="result-row">
                <span class="result-label">Est. excess consumption</span>
                <span class="result-value" id="res-excess">—</span>
              </div>
              <div class="result-row">
                <span class="result-label">Estimated reduction</span>
              ...
              <div class="results-savings-box" style="background: rgba(0,174,203,0.04);">
...
(function () {
  const inputs = {
    ...
    monthlyBill:   document.getElementById('calc-monthly-bill'),
    irrigationPct: document.getElementById('calc-irrigation-pct'),
    waterSource:   document.getElementById('calc-water-source'),
  };

  const coeffInputs = {
    palms:       document.getElementById('coeff-palms'),
    trees:       document.getElementById('coeff-trees'),
    shrubs:      document.getElementById('coeff-shrubs'),
    groundcover: document.getElementById('coeff-groundcover'),
    turf:        document.getElementById('coeff-turf'),
  };
  ...
  const resCurrentCost = document.getElementById('res-current-cost');
  const resDemandVol   = document.getElementById('res-demand-volume');
  const resEfficientCost = document.getElementById('res-efficient-cost');
  const resExcess      = document.getElementById('res-excess');
  ...
  function fmtAED(num) {
    if (num >= 1000000) return 'AED ' + fmt(num / 1000000, 2) + 'M';
    if (num >= 1000)    return 'AED ' + fmt(num / 1000, 1) + 'K';
    return 'AED ' + fmt(num);
  }

  function calculate() {
    var palms       = val(inputs.palms);
    var trees       = val(inputs.trees);
    var shrubs      = val(inputs.shrubs);
    var groundcover = val(inputs.groundcover);
    var turf        = val(inputs.turf);
    var monthlyBill = val(inputs.monthlyBill);
    var irrigPct    = val(inputs.irrigationPct) / 100;

    var c_palms       = parseFloat(coeffInputs.palms.value)       || <?php echo (float) $c_palms; ?>;
    var c_trees       = parseFloat(coeffInputs.trees.value)       || <?php echo (float) $c_trees; ?>;
    var c_shrubs      = parseFloat(coeffInputs.shrubs.value)      || <?php echo (float) $c_shrubs; ?>;
    var c_groundcover = parseFloat(coeffInputs.groundcover.value) || <?php echo (float) $c_groundcover; ?>;
    var c_turf        = parseFloat(coeffInputs.turf.value)        || <?php echo (float) $c_turf; ?>;

    if (!monthlyBill || !irrigPct) { showEmpty(); return; }

    var monthlyIrrigCost = monthlyBill * irrigPct;

    var impliedTariff = 4.5;
    var utility = document.getElementById('calc-utility') ? document.getElementById('calc-utility').value : '';
    if (utility === 'taqa')      impliedTariff = 3.8;
    else if (utility === 'dewa') impliedTariff = 5.2;
    else if (utility === 'sewa') impliedTariff = 3.5;

    var currentIrrigVolume = monthlyIrrigCost / impliedTariff;

    var totalEfficientDemand =
      (palms       * c_palms) +
      (trees       * c_trees) +
      (shrubs      * c_shrubs) +
      (groundcover * c_groundcover) +
      (turf        * c_turf);

    var hasPlantData = (palms + trees + shrubs + groundcover + turf) > 0;
    var efficientVolume, savingsPct;

    if (hasPlantData && totalEfficientDemand > 0) {
      efficientVolume = totalEfficientDemand;
      if (efficientVolume >= currentIrrigVolume) {
        efficientVolume = currentIrrigVolume * 0.70;
      }
      savingsPct = Math.max(0, 1 - efficientVolume / currentIrrigVolume);
    } else {
      savingsPct      = 0.35;
      efficientVolume = currentIrrigVolume * (1 - savingsPct);
    }

    var efficientCost  = efficientVolume * impliedTariff;
    var monthlySavings = monthlyIrrigCost - efficientCost;
    var annualSavings  = monthlySavings * 12;
    var excessVolume   = currentIrrigVolume - efficientVolume;

    resCurrentCost.textContent    = 'AED ' + fmt(monthlyIrrigCost) + ' / mo';
    resDemandVol.textContent      = hasPlantData ? fmt(efficientVolume, 0) + ' m³ / mo' : '— (enter plant data)';
    resEfficientCost.textContent  = 'AED ' + fmt(efficientCost) + ' / mo';
    resExcess.textContent         = hasPlantData ? fmt(excessVolume, 0) + ' m³ / mo' : 'Enter plant data above';
    resPct.textContent            = '~' + fmt(savingsPct * 100, 0) + '%';
    resMonthly.textContent        = fmtAED(monthlySavings) + ' / mo';
    resAnnual.textContent         = fmtAED(annualSavings) + ' / yr';
  ...
  Object.values(coeffInputs).forEach(function(input) {
  ...
  var toggleBtn = document.getElementById('toggleAssumptions');
  var panel     = document.getElementById('assumptionsPanel');
  if (toggleBtn && panel) {
    toggleBtn.addEventListener('click', function() {
      var open = panel.classList.toggle('open');
      toggleBtn.setAttribute('aria-expanded', open);
      toggleBtn.querySelector('svg').style.transform = open ? 'rotate(180deg)' : '';
    });
  }
})();
</script>
